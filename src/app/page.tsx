...
export default function Home() {
  ...
  const [activePage, setActivePage] = useState<'home' | 'products' | 'delivery' | 'contact' | 'articles'>('home');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const storedUsers = localStorage.getItem('maivzev_users_v3');
      if (storedUsers) {
        const parsed = JSON.parse(storedUsers);
        const filtered = parsed.filter((u: UserProfile) => {
          const name = (u.username || '').toLowerCase();
          const email = (u.email || '').toLowerCase();
          return !name.includes('สมจิต') && !name.includes('สมชาย') &&
            !name.includes('somjit') && !name.includes('somchai');
        });
        setUsers(filtered);
        if (filtered.length !== parsed.length) {
          localStorage.setItem('maivzev_users_v3', JSON.stringify(filtered));
        }
      } else {
        const storedUsersV2 = localStorage.getItem('maivzev_users_v2');
        if (storedUsersV2) {
          const parsed = JSON.parse(storedUsersV2);
          const filtered = parsed.filter((u: UserProfile) => {
            const name = (u.username || '').toLowerCase();
            const email = (u.email || '').toLowerCase();
            return !name.includes('สมจิต') && !name.includes('สมชาย') &&
          ...
          setUsers(mockUsers);
          localStorage.setItem('maivzev_users_v3', JSON.stringify(mockUsers));
        }
      }
      ...
      if (storedCurrentUser) {
        ...
        const name = (parsed.username || '').toLowerCase();
        if (name.includes('สมจิต') || name.includes('สมชาย') || name.includes('somjit') || name.includes('somchai')) {
          setCurrentUser(null);
        ...
        const storedCurrentUserV2 = localStorage.getItem('maivzev_current_user_v2');
        if (storedCurrentUserV2) {
          const parsed = JSON.parse(storedCurrentUserV2);
          const name = (parsed.username || '').toLowerCase();
          if (name.includes('สมจิต') || name.includes('สมชาย') || name.includes('somjit') || name.includes('somchai')) {
            setCurrentUser(null);
          } else {
            setCurrentUser(parsed);
            localStorage.setItem('maivzev_current_user_v3', JSON.stringify(parsed));
          }
        }
      }

      const storedOrders = localStorage.getItem('maivzev_orders_v3');
      if (storedOrders && JSON.parse(storedOrders).length > 0) {
        setOrders(JSON.parse(storedOrders));
      } else {
        // Seed initialOrders to localStorage so the dashboard is not blank!
        setOrders(initialOrders);
        localStorage.setItem('maivzev_orders_v3', JSON.stringify(initialOrders));
      }

      const storedProducts = localStorage.getItem('maivzev_products_v3');
      if (storedProducts) {
        setProducts(JSON.parse(storedProducts));
      } else {
        const storedProductsV2 = localStorage.getItem('maivzev_products_v2');
        if (storedProductsV2) {
          setProducts(JSON.parse(storedProductsV2));
          localStorage.setItem('maivzev_products_v3', storedProductsV2);
        } else {
          setProducts(initialProducts);
          localStorage.setItem('maivzev_products_v3', JSON.stringify(initialProducts));
        }
      }

      const storedCategories = localStorage.getItem('maivzev_categories_v3');
      if (storedCategories) {
        setCategories(JSON.parse(storedCategories));
      } else {
        const storedCategoriesV2 = localStorage.getItem('maivzev_categories_v2');
        if (storedCategoriesV2) {
          setCategories(JSON.parse(storedCategoriesV2));
          localStorage.setItem('maivzev_categories_v3', storedCategoriesV2);
        } else {
          setCategories(initialCategories);
          localStorage.setItem('maivzev_categories_v3', JSON.stringify(initialCategories));
        }
      }
      ...
      if (storedCart) {
        ...
        const storedCartV2 = localStorage.getItem('maivzev_cart_v2');
        if (storedCartV2) {
          setCart(JSON.parse(storedCartV2));
          localStorage.setItem('maivzev_cart_v3', storedCartV2);
        }
      }
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        if (currentUser) {
          localStorage.setItem('maivzev_current_user_v3', JSON.stringify(currentUser));
        } else {
          localStorage.removeItem('maivzev_current_user_v3');
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        localStorage.setItem('maivzev_products_v3', JSON.stringify(products));
      } catch (e) {
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        localStorage.setItem('maivzev_categories_v3', JSON.stringify(categories));
      } catch (e) {
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        localStorage.setItem('maivzev_cart_v3', JSON.stringify(cart));
      } catch (e) {
  ...
  const handleProfileImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        // Compress avatar to max 160x160 pixels with 0.5 quality
  ...
  const handleSignupSubmit = (e: React.FormEvent) => {
    ...
    const phoneExists = users.some((u) => u.phone && u.phone.replace(/[-\s]/g, '') === signupPhone.trim().replace(/[-\s]/g, ''));
    if (phoneExists) {
  ...
  const handleSlipChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        const base64String = reader.result as string;
        // Compress slip image to max 500x650 pixels with 0.5 quality
        const compressed = await compressImage(base64String, 500, 650, 0.5);
      ...
      <header className="sticky top-0 z-40 bg-[#166534] text-white shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            {/* Logo */}
            <div onClick={() => { setActivePage('home'); setSelectedCategory(null); }} className="flex items-center gap-3 cursor-pointer">
            ...
            <nav className="hidden md:flex items-center gap-8 text-base font-medium">
              <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('home'); setSelectedCategory(null); }} className={`${activePage === 'home' ? 'text-white underline decoration-2 underline-offset-4' : 'text-emerald-100 hover:text-emerald-200'} transition-colors`}>หน้าแรก</a>
              <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('products'); setSelectedCategory(null); }} className={`${activePage === 'products' ? 'text-white underline decoration-2 underline-offset-4' : 'text-emerald-100 hover:text-emerald-200'} transition-colors`}>สินค้า</a>
              <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('delivery'); }} className={`${activePage === 'delivery' ? 'text-white underline decoration-2 underline-offset-4' : 'text-emerald-100 hover:text-emerald-200'} transition-colors`}>วิธีสั่งซื้อ</a>
              <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('contact'); setSelectedCategory(null); }} className={`${activePage === 'contact' ? 'text-white underline decoration-2 underline-offset-4' : 'text-emerald-100 hover:text-emerald-200'} transition-colors`}>ติดต่อเรา</a>
              <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('articles'); setSelectedCategory(null); }} className={`${activePage === 'articles' ? 'text-white underline decoration-2 underline-offset-4' : 'text-emerald-100 hover:text-emerald-200'} transition-colors`}>บทความน่ารู้</a>
            </nav>

            {/* Icons */}
            <div className="flex items-center gap-4">
              {/* User Profile Button with Dropdown (Login State) */}
                      ...
                      <div>
                        ...
                        <div className="px-2 pt-2 space-y-1">
                              ...
                              onChange={(e) => {
                                ...
                                if (currentUser && file) {
                                  ...
                                  reader.onloadend = async () => {
                                    ...
                                    const updatedUser = { ...currentUser, profileImage: compressed };
                                    setCurrentUser(updatedUser);

                                    setUsers(prevUsers => prevUsers.map(u => u.email.toLowerCase() === currentUser.email.toLowerCase() ? { ...u, profileImage: compressed } : u));
                          ...
                          {currentUser.role === 'Admin' && (
                            <button
                              onClick={() => {
                                setViewMode(viewMode === 'admin' ? 'shop' : 'admin');
                                setIsUserDropdownOpen(false);
                              }}
                            ...
                            >
                              ...
                              {viewMode === 'admin' ? '🛒 เข้าสู่หน้าร้านค้า' : '⚙️ ระบบดูแลระบบ (Admin Panel)'}
                            </button>
                          )}
                          <button
                            onClick={() => {
              ...
              {currentUser && (
                <button
                  ...
                  className="relative p-2.5 hover:bg-white/10 rounded-full transition-all duration-200 focus:outline-none"
                >
                  <svg className="w-6 h-6 text-emerald-200 hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                  {totalItems > 0 && (
                    <span className="absolute top-0.5 right-0.5 bg-yellow-400 text-stone-900 text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-sm animate-pulse">
                      {totalItems}
                    </span>
                  )}
                </button>
              ...
              <button
                ...
                className="md:hidden p-2 hover:bg-white/10 rounded-full transition-colors focus:outline-none"
              >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-[#0f4c27] px-4 pt-2 pb-4 space-y-1 transition-all">
            <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('home'); setSelectedCategory(null); setIsMobileMenuOpen(false); }} className={`block px-3 py-2 rounded-md hover:bg-emerald-800 text-white font-medium ${activePage === 'home' ? 'bg-[#0b3c1e]' : ''}`}>หน้าแรก</a>
            <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('products'); setSelectedCategory(null); setIsMobileMenuOpen(false); }} className={`block px-3 py-2 rounded-md hover:bg-emerald-800 text-emerald-100 ${activePage === 'products' ? 'bg-[#0b3c1e] text-white font-medium' : ''}`}>สินค้า</a>
            <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('delivery'); setIsMobileMenuOpen(false); }} className={`block px-3 py-2 rounded-md hover:bg-emerald-800 text-emerald-100 ${activePage === 'delivery' ? 'bg-[#0b3c1e] text-white font-medium' : ''}`}>วิธีสั่งซื้อ</a>
            <a href="#" onClick={(e) => { e.preventDefault(); setActivePage('contact'); setSelectedCategory(null); setIsMobileMenuOpen(false); }} className={`block px-3 py-2 rounded-md hover:bg-emerald-800 text-emerald-100 ${activePage === 'contact' ? 'bg-[#0b3c1e] text-white font-medium' : ''}`}>ติดต่อเรา</a>
          </div>
        )}
      </header>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                ...
                <div>
                  ...
                  <ul className="space-y-2 text-sm font-medium">
                    <li><a href="#" onClick={(e) => { e.preventDefault(); setActivePage('home'); setSelectedCategory(null); }} className="hover:text-emerald-400 transition-colors">หน้าแรก</a></li>
                    <li><a href="#" onClick={(e) => { e.preventDefault(); setActivePage('products'); setSelectedCategory(null); }} className="hover:text-emerald-400 transition-colors">สินค้าทั้งหมด</a></li>
                    <li><a href="#" onClick={(e) => { e.preventDefault(); setActivePage('delivery'); }} className="hover:text-emerald-400 transition-colors">วิธีสั่งซื้อ</a></li>
                    <li><a href="#" onClick={(e) => { e.preventDefault(); setActivePage('contact'); setSelectedCategory(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="hover:text-emerald-400 transition-colors">ติดต่อเรา</a></li>
                    <li><a href="#" onClick={(e) => { e.preventDefault(); setActivePage('articles'); setSelectedCategory(null); }} className="hover:text-emerald-400 transition-colors">บทความน่ารู้</a></li>
                  </ul>
                </div>
                <div>
...
(แอดมินได้รับการแจ้งเตือนสลิปโอนเงินของคุณแล้ว และกำลังทำการตรวจสอบความถูกต้องในระบบดูแลระบบครับ)`);
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-full shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 focus:outline-none text-base"
                    >
                    ...
                    <form onSubmit={(e) => {
                      ...
                      if (forgotFoundUser) {
                        setUsers(prevUsers => prevUsers.map(u =>
                          u.email.toLowerCase() === forgotFoundUser.email.toLowerCase()
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (file) {
                                ...
                                const reader = new FileReader();
                                reader.onloadend = () => {
                                  setReviewMediaUrl(reader.result as string);
